...
namespace IOModule {
    ...
    public class RPIIO:BaseIO {

        private static KPPLogger log = new KPPLogger(typeof(RPIIO));

        internal GPIOMem MemIO;

        public delegate void IOStateChanged(RPIIO sender, Boolean NewState);
        ...
        public RPIIO(String name, GPIOPins PIN, IOType pinType) {
            Name = name;
            Pin = PIN;
            PinType = pinType;
        }
    ...
    public class IOList {
        ...
        private List<RPIIO> _IOs = new List<RPIIO>();

        public List<RPIIO> IOPins {
            get { return _IOs; }
            set { _IOs = value; }
        }
    ...
    public static class RPIGPIOObject {

        private static KPPLogger log = new KPPLogger(typeof(RPIGPIOObject));

        static private Boolean isRPI = false;
        ...
        internal static void MonitorIOS() {
            ...
            while (!Stop) {
                ...
                foreach (RPIIO item in IOs.IOPins) {
                    if (item.Active) {
        ...
        public static void Init(IOList ios, Boolean rpi) {
            try {
                isRPI = rpi;
                IOs = ios;
                if (isRPI) {
                    foreach (RPIIO item in ios.IOPins) {
                        ...
                        if (item.Active) {
                            RPIIO sameIO = ios.IOPins.Find(io => (io.Pin == item.Pin && io.MemIO != null));
                            if (sameIO != null) {
